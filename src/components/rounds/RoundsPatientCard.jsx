...
import { User, Calendar, Shield, Bed, AlertTriangle, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className={cn(
          ...
          <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
            ...
            {pendingCount > 0 && (
              <span className="text-[9px] bg-primary text-primary-foreground font-bold px-1.5 py-0.5 rounded-full animate-pulse">
                {pendingCount} AI note{pendingCount > 1 ? 's' : ''} pending
              </span>
            )}
          </div>
        </div>
      ...
      {notes.length > 0 && (
        <>
          ...
          >
            <span>{notes.length} AI note{notes.length > 1 ? 's' : ''}{pendingCount > 0 ? ` · ${pendingCount} pending review` : ''}</span>
